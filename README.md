...
Что происходит под капотом: OAuth с **PKCE**. Секретная строка (`code_verifier`) не покидает
ваш компьютер, а в переписку попадает только одноразовый код, действующий 10 минут — без
`code_verifier` он бесполезен, поэтому его утечка не открывает доступ к вашей аналитике.
Запрашивается единственное право — «Получение статистики, чтение параметров своих и доверенных
счётчиков» (`metrika:read`).

Токен сохраняется в `~/.config/mcp-yandex-metrica/credentials.json` с правами `0600` (только
владелец) вместе с `refresh_token`, поэтому доступ продлевается сам и не отваливается через год.
Проверить состояние — попросите «покажи статус подключения» (`auth_status`), отключить —
«отключи Метрику» (`logout`). Выданный доступ отзывается в
